...
export class LeagueService {
  ...
  createLeague(
    ...
    show: string,
    seasonNumber: number
  ): Observable<CreateLeagueResponse> {
  ...
  getLeagueById(leagueId: string, forceRefresh = false): Observable<League> {
    if (!forceRefresh) {
      const cachedLeagues = this.cacheService.get<League[]>('leagues');
      if (cachedLeagues) {
        const cachedLeague = cachedLeagues.find(
          (league: League) => league.leagueId! === leagueId
        );
        if (cachedLeague) {
    ...
    return this.apiService.league.getLeagueById(leagueId).pipe(
      ...
      tap((fetchedLeague: League) => {
        ...
        const updatedLeagues = [
          ...cachedLeagues.filter(
            (league: League) => league.leagueId! !== leagueId
          ), // Remove any existing entry for this leagueId
      ...
      catchError((error: any) => {
        console.error(
          `Error fetching league by ID (${leagueId}):`,
          error.response?.data || error.message
        );
        return throwError(() => error);
      })
  ...
  getInvitesForLeague(
    ...
    if (!forceRefresh) {
      const cachedInvites = this.cacheService.get<Invite[]>(
        `invites_${leagueId}`
      );
      if (cachedInvites) {
    ...
    return this.apiService.league.getInvitesForLeague(leagueId).pipe(
      ...
      catchError((error: any) => {
        console.error(
          'Error fetching invites for league:',
          error.response?.data || error.message
        );
        return throwError(() => error);
      })
  ...
  inviteUserToLeague(
    ...
    return this.apiService.league
      .inviteUserToLeague(leagueId, { inviteeEmail })
      .pipe(
        take(1),
        tap((response: InviteUserToLeagueResponse) => {
          const newInvite = response.invite!;
          const invites =
            this.cacheService.get<Invite[]>(`invites_${leagueId}`) || [];
