...
import { Season, Show } from '../../../libs/generated/typescript-angular';
...
export class CreateLeagueFormComponent implements OnInit {
  ...
  buildSeasonMenuItems(selectedShowName: string) {
    const selectedShow = this.shows.find(show => show.name === selectedShowName);
    this.seasonMenuItems = selectedShow?.seasons?.map(season => ({
