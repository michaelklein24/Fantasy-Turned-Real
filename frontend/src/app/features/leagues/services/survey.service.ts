...
import { CreateSurveyRequest, CreateSurveyResponse, GetSurveyByIdResponse, GetSurveysResponse, Survey, UpdateSurveyRequest } from '../../../../libs/generated/typescript-angular';
...
export class SurveyService {
  ...
  public getSurveyById(surveyId: string): Observable<Survey> {
    return this.apiService.survey.getSurveyById(surveyId).pipe(
      ...
      catchError((error: any) => {
        console.error('Error fetching survey by ID:', error.response?.data || error.message);
        return throwError(() => error);
      })
    );
  }

  public createSurvey(leagueId: string, name: string, startDate: Date, endDate: Date): Observable<Survey> {
  ...
  public updateSurveyDetails(surveyId: string, name: string, startDate: Date, endDate: Date): Observable<void> {
    const start: string = startDate.toISOString().split('T')[0];
    const end: string = endDate.toISOString().split('T')[0];
  ...
  public deleteSurvey(surveyId: string): Observable<void> {
    return this.apiService.survey.deleteSurvey(surveyId).pipe(
      catchError((error: any) => {
        console.error(`Error deleting survey with ID ${surveyId}:`, error.response?.data || error.message);
        return throwError(() => error);
      })
    );
  }
